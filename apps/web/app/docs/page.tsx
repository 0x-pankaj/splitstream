  ...
  return (
    <div>
      <button
        ...
        className="rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-brandhover"
      >
        ➕ Add Arc Testnet to my wallet
      </button>
      {status !== "idle" ? (
        <div className="mt-2 text-xs" style={{ color: status === "ok" ? "#0E7A56" : "#C2410C" }}>{msg}</div>
      ) : null}
    </div>
  );
}
...
function Code({ children, label }: { children: string; label?: string }) {
  ...
  return (
    <div className="relative mt-2">
      {label ? <div className="mb-1 text-[11px] uppercase tracking-wider text-faint">{label}</div> : null}
      <button
        onClick={copy}
        className="absolute right-2 top-2 z-10 rounded-md border border-line3 bg-surface px-2 py-1 text-[11px] text-ink3 hover:bg-chip"
      >
        {copied ? "Copied" : "Copy"}
      </button>
      <pre className="mono overflow-x-auto rounded-lg border border-line bg-surface-2 p-3 pr-16 text-xs leading-relaxed text-ink2">
        {children}
      </pre>
    </div>
  );
...
function H({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h2 id={id} className="scroll-mt-24 text-lg font-semibold text-ink">
      {children}
    </h2>
  );
}
    ...
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3 sm:mb-8">
        <Link href="/" className="flex items-center gap-2">
          <span className="text-lg font-semibold tracking-tight text-ink">SplitStream</span>
          <Pill text="docs" tone="slate" />
        </Link>
        <div className="flex items-center gap-2">
          <Link href="/publish" style={{ background: "#EE5126" }} className="rounded-lg px-3 py-2 text-xs font-semibold text-white hover:bg-brandhover">+ Publish</Link>
          <Link href="/" className="rounded-lg border border-line3 bg-surface px-3 py-2 text-xs text-ink hover:bg-chip">Storefront →</Link>
        </div>
      </header>
        ...
        <nav className="hidden lg:block">
          <div className="sticky top-8 space-y-1 text-sm">
            {NAV.map((n) => (
              <div key={n.id}>
                {n.group ? (
                  <div className="mt-4 mb-1 text-[11px] uppercase tracking-wider text-faint">{n.group}</div>
                ) : null}
                <a href={`#${n.id}`} className="block rounded px-2 py-1 text-muted hover:bg-chip hover:text-ink">
                  {n.label}
                </a>
              </div>
            ))}
        ...
        <div className="space-y-10">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Card>
                <div className="mb-1 text-sm font-semibold text-ink">You list a service →</div>
                <p className="text-sm text-muted">Register content or an API endpoint with a price and a revenue split. Integrate nothing — we are the paywall.</p>
              </Card>
              <Card>
                <div className="mb-1 text-sm font-semibold text-ink">Someone pays per use →</div>
                <p className="text-sm text-muted">A human clicks, or an AI agent pays autonomously per call in USDC. The split settles to all owners across chains.</p>
              </Card>
            </div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <Card>
              <ul className="space-y-2 text-sm text-ink3">
                <li><strong className="text-ink">Piece</strong> — the unit you list. <code className="mono text-brand">kind</code> is <code className="mono">article·photo·song·podcast</code> (content) or <code className="mono">api</code> (a paid endpoint).</li>
                <li><strong className="text-ink">Split</strong> — contributors each take a share in basis points (must sum to 10000 = 100%); each is paid on their <code className="mono">targetChain</code>.</li>
                <li><strong className="text-ink">x402</strong> — the HTTP &quot;402 Payment Required&quot; flow agents use to pay per call: <em>request → 402 challenge → pay USDC → retry with proof → get result</em>.</li>
                <li><strong className="text-ink">Credential injection</strong> — for authenticated APIs, you store the upstream key once; we inject it per call. The buyer gets access, never the key (no KYC).</li>
              </ul>
            </Card>
          </section>
          ...
          <section className="space-y-3">
            ...
            <p className="text-sm text-ink3">Open a publisher account once to get a scoped key (the demo key <code className="mono text-brand">arc_test_sk_demo_0001</code> works out of the box).</p>
...
# → { apiKey: "arc_test_sk_...", ... }  (shown once)`}</Code>
          ...
          <section className="space-y-3">
            ...
            <p className="text-sm text-ink3">A reader pays a few cents to unlock; the price fans out to every contributor on their chain. Use the <Link href="/publish" className="text-brand hover:text-brandhover">/publish</Link> form, or the API:</p>
  ...
  -d '{
    ...
    "contributors": [
      ...
      { "role":"photographer", "address":"0x3333…", "targetChain":"ethereum", "splitBps":1500 }
            ...
            <p className="text-sm text-muted">It&apos;s now live in the catalog and shareable at <code className="mono">/piece/&lt;id&gt;</code>.</p>
          </section>

          <section className="space-y-3">
            <H id="sell-api">3. List a paid API (x402 pay-per-call)</H>
            <p className="text-sm text-ink3">Register any HTTP endpoint with a per-call price. You integrate nothing — SplitStream becomes the x402 paywall and proxies the call on payment.</p>
  ...
  -d '{
    ...
    "contributors": [{ "role":"owner", "address":"0xYourArcEvmAddress", "targetChain":"base", "splitBps":10000 }]
            ...
            <p className="text-sm text-muted">Multi-owner APIs split automatically — list several contributors.</p>
          </section>

          <section className="space-y-3">
            <H id="sell-auth">4. Authenticated APIs — access without KYC</H>
            <p className="text-sm text-ink3">If your endpoint needs a key, store it once with the piece. It&apos;s <strong>write-only</strong> — never returned by any read endpoint — and injected per call. The paying agent gets the result, never the key.</p>
...
"auth": { "type":"query",  "name":"apikey",    "secret":"…" }`}</Code>
            <p className="text-sm text-muted">Reads expose only <code className="mono">authenticated:true</code> and <code className="mono">authType</code> — never the secret.</p>
          </section>
          ...
          <section className="space-y-3">
            ...
            <Card>
              <ul className="space-y-2 text-sm text-ink3">
                <li><strong className="text-ink">Live (LIVE_X402)</strong> — the buyer&apos;s USDC payment is verified on Arc, then each owner is paid their split in <strong>real USDC on Arc</strong>. Proven end-to-end (owner balance 0 → 0.01 on-chain).</li>
                <li><strong className="text-ink">Mirror (default)</strong> — keyless demo: simulated settlement so anyone can try it instantly. Split math, credential injection, and live upstream calls are real either way.</li>
                <li><strong className="text-ink">Cross-chain</strong> — EVM contributors are paid on Arc directly; other chains route via Circle CCTP / Gateway.</li>
              </ul>
            </Card>
          </section>
...
AI:    "It's about €0.86 to the dollar."`}</Code>
          ...
          <section className="space-y-3">
            ...
            <p className="text-sm text-ink3">No SplitStream-specific install. An agent with an x402 client + a funded Arc wallet pays our endpoint the standard way. This is the same model the wider x402 ecosystem (incl. pay.sh) uses; Circle&apos;s <code className="mono">pay-via-agent-wallet</code> does exactly this on Arc.</p>
...
→ 200 OK
  ...
  { "paid":true, "mode":"live-arc", "payments":[…], "upstream":{ "ok":true, "body":{…} } }`}</Code>
            <p className="text-sm text-muted mt-2">Try the full handshake locally: <code className="mono">pnpm --filter @arcane/server x402:call</code>. The single-use nonce blocks replay.</p>
          </section>
...
# → { unlock: { contributors:[ …paid per chain… ] } }`}</Code>
          ...
          <section className="space-y-3">
            ...
            <Card>
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="w-full text-left text-xs text-ink3">
                  <tbody className="mono">
                    <tr><td className="py-1 pr-4 text-faint">Network name</td><td className="py-1">Arc Testnet</td></tr>
                    <tr><td className="py-1 pr-4 text-faint">RPC URL</td><td className="py-1">https://rpc.testnet.arc.network</td></tr>
                    <tr><td className="py-1 pr-4 text-faint">Chain ID</td><td className="py-1">5042002</td></tr>
                    <tr><td className="py-1 pr-4 text-faint">Currency symbol</td><td className="py-1">USDC</td></tr>
                    <tr><td className="py-1 pr-4 text-faint">Block explorer</td><td className="py-1">https://testnet.arcscan.app</td></tr>
                    <tr><td className="py-1 pr-4 text-faint">USDC (ERC-20)</td><td className="py-1">{ARC_USDC}</td></tr>
                  </tbody>
                </table>
              </div>
              <p className="mt-3 text-xs text-muted">
                ...
                <a className="text-brand hover:text-brandhover" target="_blank" rel="noreferrer" href="https://faucet.circle.com">Circle faucet</a>{" "}
          ...
          <section className="space-y-3">
            ...
            <Card>
              ...
              <p className="mt-3 text-xs text-faint">tRPC equivalents: <code className="mono">pieces.list / get / create / unlock / callApi</code>, <code className="mono">traction.stats</code>, <code className="mono">agent.read</code>. MCP tools: <code className="mono">list_pieces, call_api, pay_for_piece</code>.</p>
            </Card>
          </section>
          ...
          <section className="space-y-3">
            ...
            <Card>
              <ul className="space-y-2 text-sm text-ink3">
                <li><strong className="text-ink">402 body</strong> — <code className="mono">{`{ x402Version, accepts:[ PaymentRequirements ] }`}</code></li>
                <li><strong className="text-ink">PaymentRequirements</strong> — <code className="mono">scheme:&quot;exact&quot;, network:&quot;arc-testnet&quot;, maxAmountRequired (atomic USDC), payTo, asset, nonce, resource, maxTimeoutSeconds</code></li>
                <li><strong className="text-ink">X-PAYMENT</strong> (request header, base64) — <code className="mono">{`{ x402Version, scheme, network, payload:{ nonce, from, authorization } }`}</code></li>
                <li><strong className="text-ink">X-PAYMENT-RESPONSE</strong> (response header, base64) — <code className="mono">{`{ success, transaction, network, payer }`}</code></li>
              </ul>
            </Card>
          </section>
          ...
          <section className="space-y-3">
            ...
            <Card>
              <ul className="space-y-1.5 text-sm text-ink3">
                <li>Chain: <strong className="text-ink">Arc Testnet</strong> · id <code className="mono">5042002</code> · explorer <code className="mono">testnet.arcscan.app</code></li>
                <li>USDC (6-dp ERC-20): <code className="mono">{ARC_USDC}</code> — also Arc&apos;s native gas token</li>
                <li><strong className="text-ink">Mirror</strong> mode (default): keyless, simulated settlement. <strong className="text-ink">LIVE_X402</strong>: real USDC on Arc (funded relayer).</li>
              </ul>
            </Card>
          </section>
